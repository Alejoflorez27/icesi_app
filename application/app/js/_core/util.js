...
function process_error_response(xhr) {
    if (xhr.status == 400 || xhr.status == 401) {
        r = JSON.parse(xhr.responseText)
        alertSwal('error', 'Error', r.code.code);
        if (xhr.status == 401)
...
function fullScreen(fullScreen) {
    if (fullScreen) {
        document.documentElement.requestFullscreen().then((r)=>{console.log(r)}).catch((e)=>{console.error(e)});
    } else {
