$(document).ready(function () {
    ...
    //loadSelectOption para cargar la lista de los Tipo de Contrato
    loadSelectOption({
    ...
    $('#btn-submit-laborall').on('click', function (e) {
        ...
        if(validateFormLaboralEdit()){
            ...
            let data = {                                //Se Prepara la Data que va a ser modificada en la BD
                ...
                motivo_retiro: $('#motivo_retiro_edit').val(),
            ...
            $.ajax({
                ...
                url: url_site(`api/laboral/update_laboral_visita_ingreso?id_laboral=${data.id_laboral}`), //Se le da el id_candidato de la data
                contentType: 'application/json',
                data: JSON.stringify(data),
                dataType: "json",
...
function validateFormlaboral() {
    ...
    if ($('#tipo_retiro').val() == "" || $('#tipo_retiro').val() == null) {
        alertSwal('error', 'Tipo retiro', 'Este campo es obligatorio');
        $("#tipo_retiro").focus();
        return false;
    }
    return true;
...
function cargarTablaLaboral() {
    ...
    $.ajax({
        ...
        type: "GET",
        url: url_site(`api/laboral/descripcion_visitas?id_solicitud=${id_solicitudl}&id_servicio=${id_servicioC}`),
        dataType: "json",
        success: function (r) {
