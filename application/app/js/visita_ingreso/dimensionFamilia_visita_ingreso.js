$(document).ready(function () {
    ...
    var id_dimension = $('#id_dimension').val(); //Dimension de familia
    ...
    loadSelectOption({
        url: url_site(`api/dimensiones/variables_dimensiones?id_servicio=${id_servicioC}&id_dimension=${id_dimension}`),//Dimension de familia
        input: [{
            id: 'variableFamilia',                      //Nombre del campo en HTML
            clearOptions: true,
    ...
    loadSelectOption({
        url: url_site(`api/dimensiones/variables_dimensiones?id_servicio=${id_servicioC}&id_dimension=${id_dimension}`),//Dimension de familia
        input: [{
            id: 'variableFamiliaEdit',                      //Nombre del campo en HTML
            clearOptions: true,
            emptyText: 'Seleccione la variable',
            selectedValue: ''
        },
    ...
    cargarTablaFamiliaDimension();

    //envio de Accion POST
   $('.btnAddDimensionFamilia').on('click', function () {
        $('#accion').val("POST");
    });

    //Envio de Accion PUT
    $('#btn-submit-dimensionEditFamilia').on('click', function () {
        //$('#formDimensionFamiliaedit')[0].reset();
        $('#accion').val("PUT");
    });

    // Enviar id_solicitud a familia_visita_ingreso
    $('#btn-submit-Anterior').on('click',function () {
   
        window.location = url_site(`laboral_visita_ingreso?id_solicitud=${id_solicitudC}&id_servicio=${id_servicioC}`)
    })
...
$('#btn-submit-Siguiente').on('click', function () {
    ...
    $('#variableFamilia').on('change', function () {
        let id_pregunta = $('#variableFamilia').val();            //Se Crea una variable para guardar lo que pinto en el campo pais para mandarlo por parametro
        //console.log (variableFamilia);
    ...
    $('#btn-submit-dimensionFamilia').on('click', function (e) {
        e.preventDefault(e);
        ...
        if (validateFormFamiliaDimension()) {
            ...
            let data = {
                id_pregunta: $('#variableFamilia').val(),
            ...
            $.ajax({
                ...
                success: function (r) {
                    //console.log(r);
                    if (r.status == "success") {
                        alertSwal('success', 'Dimension Familia', 'Variable guardada satisfactoriamente');
                        $('#btn-submit-dimensionFamilia').prop('disabled', false);
                        cargarTablaFamiliaDimension();
                        $('#formDimensionFamilia')[0].reset();
                        //$('#modalAddServicio').modal('hide')
                        //loadSelectOption para cargar la lista de las variables
                        loadSelectOption({
                            url: url_site(`api/dimensiones/variables_dimensiones?id_servicio=${id_servicioC}&id_dimension=${id_dimension}`),
                            input: [{
                                id: 'variableFamilia',                      //Nombre del campo en HTML
                                clearOptions: true,
                                emptyText: 'Seleccione la variable',
    ...
    $('.btnPlantilla').on('click', function (e) {
        e.preventDefault(e);
        let id_pregunta = $('#variableFamilia').val();
    ...
    $('#tbl-familiaDimension').on('click', '.btnEditDimensionFamilia', function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                $('#variableFamiliaEdit').val(resp.data.id_pregunta);
                $('#nivel_riesgoEdit').val(resp.data.nivel_riesgo);
                $('#respuestaEdit').val(resp.data.respuesta);
                $('#variableFamiliaEdit').prop("disabled", true);
    ...
    */
    ...
    $('#btn-submit-dimensionEditFamilia').on('click', function (e) {
        e.preventDefault();
        if(validateFormFamiliaEditDimension()){
            ...
            let data = {                                //Se Prepara la Data que va a ser modificada en la BD
                ...
                id_pregunta: $('#variableFamiliaEdit').val(),
    ...
    $("#tbl-familiaDimension").on("click", ".btnEliminarDimensionFamilia", function () {
        $('#accion').val("DELETE");
    })//Fin accion delete

    // Función de eliminar variable familiar
    $("#tbl-familiaDimension").on("click", ".btnEliminarDimensionFamilia", function (e) {
        e.preventDefault();
        ...
        Swal.fire({
            title: '¿Estás seguro?',
            text: "Esta acción eliminará la variable familiar de forma permanente.",
            icon: 'warning',
...
//*********************************************************************** */
 ...
 $('#variableFamilia').on('change', function() {
    ...
    // $('#formDimensionFinanciero')[0].reset();
     var id_pregunta = $('#variableFamilia').val();
     let params = new URLSearchParams(location.search);
     let id_solicitudC = params.get('id_solicitud');
...
function validateFormFamiliaDimension() {
    ...
    if ($('#variableFamilia').val() == "" || $('#variableFamilia').val() == null) {
...
function validateFormFamiliaEditDimension() {
    let accion = $('#accion').val();
    let expreg = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])([A-Za-z\d$@$!%*?&]|[^ ]){8,15}$/;

    if ($('#variableFamiliaEdit').val() == "" || $('#variableFamiliaEdit').val() == null) {
        alertSwal('error', 'variable Familia', 'Este campo es obligatorio');
        $("#variableFamiliaEdit").focus();
        return false;
    }
...
function cargarTablaFamiliaDimension() {
    let params = new URLSearchParams(location.search);
    ...
    $.ajax({
        ...
        type: "GET",
        url: url_site(`api/dimensiones/descripcion?id_solicitud=${id_solicitudl}&id_dimension=${id_dimension}&id_servicio=${id_servicioC}`), //Dimension de familia
        dataType: "json",
        success: function (r) {

            $('#tbl-familiaDimension').DataTable().clear();
            $('#tbl-familiaDimension').DataTable().destroy();

            let t = $('#tbl-familiaDimension').DataTable({
                paging: true,
                ordering: true,
                info: false,
                searching: true,
            ...
            if (r.status == "success") {
                ...
                r.data.forEach((sol) => {
                    ...
                    t.row.add([
                        ...
                        sol.respuesta,
                        `<button class="btn btn-xs btn-warning btnEditDimensionFamilia" id_respuesta="${sol.id_respuesta}"><i class="fa fa-pencil"  aria-hidden="true"></i> Editar</button>
                        <button class="btn btn-xs btn-danger btnEliminarDimensionFamilia" id_respuesta=${sol.id_respuesta}><i class="fa fa-trash"></i> Eliminar</button>`,
                        sol.usr_create,
                        sol.fch_create
                    ]);
                });
...
//Fin de la tabla de dimension familiar
