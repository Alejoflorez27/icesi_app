...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        case 'post_test_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrPosTestPolPre::posTestPolPreById($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
        default:
            /*
    ...
    switch (strtolower($recurso)) {
        ...
        case 'crear_post_test_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPosTestPolPre::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_post_test,
                    $v_comportamiento,
    ...
    switch (strtolower($recurso)) {
        ...
        case 'update_post_test_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPosTestPolPre::update(
                    $v_id_post_test,
                    $v_post_test,
                    $v_comportamiento,
                    $v_tipo_datos,
