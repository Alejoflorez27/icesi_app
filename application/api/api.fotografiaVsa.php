...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            echo json_encode(CtrFotografiaVsa::findByIdInfo($router->param('id_fotografia')));
            return;
            break;
        ...
        case 'lista':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrFotografiaVsa::findAll($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
        ...
        case 'validar_variable':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrFotografiaVsa::ExiteVariable($router->param('descripcion'), $router->param('id_solicitud')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case 'delete_foto':
            ...
            http_response_code(200);
            echo json_encode(
                CtrFotografiaVsa::delete(
                    $router->param('id_fotografia'),
                )
            );
            return;
            break;
