...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        */
        ...
        case 'ilicitos_pol_rutina':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrPreguntaIlicitoPolRutina::ilicitosPolRutinaById($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        ...
        case 'crear_ilicito_pol_rutina':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntaIlicitoPolRutina::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_pregunta_uno,
                    $v_pregunta_dos,
    ...
    switch (strtolower($recurso)) {
        ...
        case 'update_ilicitos_pol_rutina':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntaIlicitoPolRutina::update(
                    $v_id_preg_ilicito_pol,
                    $v_pregunta_uno,
                    $v_pregunta_dos,
                    $v_pregunta_tres
                )
