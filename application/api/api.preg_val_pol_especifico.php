...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            echo json_encode(CtrPregValPolEspecifico::findByIdValidacion($router->param('id_validacion')));
            return;
            break;
        ...
        case 'lista':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrPregValPolEspecifico::findAll($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case 'delete_validacion':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPregValPolEspecifico::delete(
                    $router->param('id_validacion'),
                )
            );
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case 'update_validacion':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPregValPolEspecifico::update(
                    $router->param('id_validacion'),
                    $v_pregunta,
                    $v_respuesta,
                )
