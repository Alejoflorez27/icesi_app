...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        case 'resultado_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrResultadoPolPre::resultadoPolPreById($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
        default:
            /*
    ...
    switch (strtolower($recurso)) {
        ...
        case 'crear_resultado_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrResultadoPolPre::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_resultado
                )
    ...
    switch (strtolower($recurso)) {
        ...
        case 'update_resultado_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrResultadoPolPre::update(
                    $v_id_resultado,
                    $v_resultado
                )
            );
