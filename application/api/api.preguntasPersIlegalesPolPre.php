...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        */
        ...
        case 'pers_ilegales_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrPreguntaPersIlegalesPolPre::personasIlegalesPolPreById($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        ...
        case 'crear_pers_ilegales_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntaPersIlegalesPolPre::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_pregunta_uno,
                    $v_pregunta_dos,
    ...
    switch (strtolower($recurso)) {
        ...
        case 'update_pers_ilegales_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntaPersIlegalesPolPre::update(
                    $v_id_preg_pers_ilegales_pol,
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_pregunta_uno,
                    $v_pregunta_dos,
