...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            echo json_encode(CtrFacCuentasCont::findById($router->param('id_cuenta')));
            return;
            break;
        ...
        case 'lista':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrFacCuentasCont::findAll(
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            if ($router->param('id_cuenta') != $v_id_cuenta) {
                echo json_encode(Result::error(__FUNCTION__, "Recurso no encontrado"));
                return;
                break;
            }

            echo json_encode(CtrFacCuentasCont::delete($router->param('id_cuenta')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            http_response_code(200);
            echo json_encode(
                CtrFacCuentasCont::update(
                    $router->param('id_cuenta'),
                    $v_item,
                    $v_concepto,
                    $v_combo,
                    $v_ubicacion_cuenta,
        ...
        case 'cambio-estado':
            ...
            echo json_encode(CtrFacCuentasCont::cambiarEstado($router->param('id_cuenta'), $v_estado_nuevo));
            return;
            break;
