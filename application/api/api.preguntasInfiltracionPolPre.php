...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        case 'infiltracion_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrPreguntaInfiltracionPolPre::infiltracionPolPreById($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        ...
        case 'crear_infiltracion_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntaInfiltracionPolPre::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_pregunta_uno,
                )
...
*/
    ...
    switch (strtolower($recurso)) {
        ...
        case 'update_infiltracion_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntaInfiltracionPolPre::update(
                    $v_id_preg_infiltracion_pol,
                    $v_pregunta_uno,
                )
            );
            return;
