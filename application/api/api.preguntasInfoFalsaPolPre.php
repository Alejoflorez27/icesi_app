...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        case 'info_falsa_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrPreguntaInfoFalsaPolPre::infoFalsaPolPreById($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        ...
        case 'crear_info_falsa_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntaInfoFalsaPolPre::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_pregunta_uno,
                    $v_pregunta_dos,
    ...
    switch (strtolower($recurso)) {
        ...
        case 'update_info_falsa_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntaInfoFalsaPolPre::update(
                    $v_id_preg_info_falsa_pol,
                    $v_pregunta_uno,
                    $v_pregunta_dos,
                    $v_id_solicitud,
                    $v_id_servicio,
