...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        case 'sol_auto':
            ...
            echo json_encode(CtrSolCandidato::SolAutoById($router->param('id_solicitud')));
            return;
            break;
        case 'sol_auto_servicio':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrSolCandidato::SolAutoServicioById($router->param('id_solicitud')));
            return;
            break;
        default:
            /*
    ...
    switch (strtolower($recurso)) {
        ...
        case 'crear_tecnica_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrtecnicaPolPre::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_pregunta_uno,
                    $v_pregunta_dos,
    ...
    switch (strtolower($recurso)) {
        ...
        case 'update_tecnica_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrtecnicaPolPre::update(
                    $v_id_tecnica,
                    $v_pregunta_uno,
                    $v_pregunta_dos,
                    $v_pregunta_tres
