...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            echo json_encode(CtrSeguridadInformaticaVsa::findByIdInfo($router->param('id_seguridad')));
            return;
            break;
        ...
        case 'lista':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrSeguridadInformaticaVsa::findAll($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
        case 'validar_variable':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrSeguridadInformaticaVsa::ExiteVariable($router->param('descripcion'), $router->param('id_solicitud')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            http_response_code(200);
            echo json_encode(
                CtrSeguridadInformaticaVsa::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_requisito,
                    $v_calificacion,
    ...
    switch (strtolower($recurso)) {
        case 'delete_foto':
            ...
            http_response_code(200);
            echo json_encode(
                CtrSeguridadInformaticaVsa::delete(
    ...
    switch (strtolower($recurso)) {
        case 'update_foto':
            ...
            http_response_code(200);
            echo json_encode(
                CtrSeguridadInformaticaVsa::update(
                    $router->param('id_seguridad'),
                    $v_requisito,
                    $v_calificacion,
                    $v_observacion,
