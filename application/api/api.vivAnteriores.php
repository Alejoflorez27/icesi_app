...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            echo json_encode(CtrVivAnteriores::findByIdVivAnteriores($router->param('id_viv_anterior')));
            return;
            break;
        ...
        case 'lista_visitas':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrVivAnteriores::findAllVisitas($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case 'delete_viv_anteriores':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrVivAnteriores::delete(
                    $router->param('id_viv_anterior'),
                )
            );
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case 'update_viv_anteriores':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrVivAnteriores::update(
                    $router->param('id_viv_anterior'),
                    $v_ubicacion,
                    $v_Tiempo_estadia,
                    $v_motivo_cambio
