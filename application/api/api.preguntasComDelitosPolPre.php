...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        case 'pro_drogas_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrPreguntaComDelitosPolPre::produccioDrogasPolPreById($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        ...
        case 'crear_com_delitos_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntaComDelitosPolPre::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_pregunta_uno,
                    $v_pregunta_dos,
...
*/
    ...
    switch (strtolower($recurso)) {
        ...
        case 'update_com_delitos_pol_pre':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntaComDelitosPolPre::update(
                    $v_id_preg_com_delitos_pol,
                    $v_pregunta_uno,
                    $v_pregunta_dos,
                    $v_pregunta_tres,
                    $v_pregunta_cuatro,
