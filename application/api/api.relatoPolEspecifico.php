...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        case 'relato_pol_especifico':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrRelatoPolEspecifico::relatoPolEspecificoById($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
        default:
            /*
    ...
    switch (strtolower($recurso)) {
        ...
        case 'crear_relato_pol_especifico':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrRelatoPolEspecifico::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_relato
                )
    ...
    switch (strtolower($recurso)) {
        ...
        case 'update_relato_pol_especifico':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrRelatoPolEspecifico::update(
                    $v_id_relato,
                    $v_relato
                )
            );
