...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        case 'proposito_pol_especifico':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrPropositoPolEspecifico::propositoPolEspecificoById($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
        default:
            /*
    ...
    switch (strtolower($recurso)) {
        ...
        case 'crear_proposito_pol_especifico':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPropositoPolEspecifico::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_proposito
                )
    ...
    switch (strtolower($recurso)) {
        ...
        case 'update_proposito_pol_especifico':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPropositoPolEspecifico::update(
                    $v_id_proposito,
                    $v_proposito
                )
            );
