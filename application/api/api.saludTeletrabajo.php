...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            echo json_encode(CtrSaludTeletrabajo::findByIdSalud($router->param('id_salud')));
            return;
            break;
        ...
        case 'lista':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrSaludTeletrabajo::findAll($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
        ...
        case 'validar_variable':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrSaludTeletrabajo::ExiteVariable($router->param('id_pregunta'), $router->param('id_solicitud')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case 'delete_salud':
            ...
            http_response_code(200);
            echo json_encode(
                CtrSaludTeletrabajo::delete(
                    $router->param('id_salud'),
                )
            );
            return;
            break;
