...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            echo json_encode(CtrPreguntasRelevantesPolPre::findByIdPreguntaRelevante($router->param('id_preg_relevante'), $router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
        ...
        case 'lista':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrPreguntasRelevantesPolPre::findAll($router->param('id_solicitud'), $router->param('id_servicio')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            http_response_code(200);
            echo json_encode(
                CtrPreguntasRelevantesPolPre::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_tipo_rn,
                    $v_pregunta,
    ...
    switch (strtolower($recurso)) {
        case 'delete_preguntas_relevantes':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntasRelevantesPolPre::delete(
                    $router->param('id_preg_relevante'),
                )
            );
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case 'update_preguntas_relevantes':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrPreguntasRelevantesPolPre::update(
                    $router->param('id_preg_relevante'),
                    $v_tipo_rn,
                    $v_pregunta,
                    $v_resp_candidato,
