...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        ...
        case 'dim_validar_existe':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrDimVarRespuestaSalud::consultaExiste($router->param('id_solicitud'), $router->param('id_pregunta')));
            return;
            break;

        case 'variable_justificacion':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrDimVarRespuestaSalud::consultaVariableSalud($router->param('id_solicitud'), $router->param('id_pregunta')));
            return;
            break;
   ...
   /* switch (strtolower($recurso)) {
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            http_response_code(200);
            echo json_encode(
                CtrDimVarRespuestaSalud::update($router->param('id_var_respuesta_salud'), $router->param('activo')
                )
            );
            return;
            break;
