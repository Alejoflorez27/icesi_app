...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            echo json_encode(CtrVivIngresos::findByIdIngresos($router->param('id_ingreso')));
            return;
            break;
        ...
        case 'lista':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrVivIngresos::findAll($router->param('id_solicitud'),$router->param('id_servicio')));
            return;
            break;
        ...
        case 'lista_noexists':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(CtrVivIngresos::findAllNoExists($router->param('tipo_parentesco')));
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case '':
            ...
            http_response_code(200);
            echo json_encode(
                CtrVivIngresos::crear(
                    $v_id_solicitud,
                    $v_id_servicio,
                    $v_tipo_familiar,
                    $v_valor_ingreso,
                    //$v_valor_aporte,
                    $v_ingreso_proveniente
                )
            );
            return;
            break;
    ...
    switch (strtolower($recurso)) {
        case 'delete_ingresos':
            header("Content-Type: application/json; charset=UTF-8");
            http_response_code(200);
            echo json_encode(
                CtrVivIngresos::delete(
                    $router->param('id_ingreso'),
                )
            );
            return;
            break;
