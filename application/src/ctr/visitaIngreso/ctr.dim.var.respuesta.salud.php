...
class CtrDimVarRespuestaSalud
{
    ...
    public static function consultaVariableSalud($id_solicitud, $id_pregunta){
        if (!isset($id_solicitud) || $id_solicitud == "")
        ...
        $result = QuerySQL::select(
            <<<SQL
                select dr.*, c.descripcion as descripcion_riesgo  
                from dim_respuestas dr, configurations c  
                where dr.id_solicitud = :id_solicitud
                and dr.id_pregunta = :id_pregunta
                and c.codigo = dr.nivel_riesgo
