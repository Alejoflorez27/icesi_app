...
//controlador que dibuja el Informe de Visita Ingreso.
class CtrInfVisIng extends PDF
{
    public static function imprimir($id_solicitud, $id_servicio)
    {
        if (isset($id_solicitud) && $id_solicitud != "" && $id_solicitud != null) {
            ...
            $pdf->SetFont('Arial','B',8);
            $pdf->Cell(45,5,utf8_decode('Apellido del Candidato:'),1,0,'L',1);
            $pdf->SetFont('Arial','',7);
            ...
            $pdf->SetFont('Arial','B',8);
            $pdf->Cell(45,5,utf8_decode('Salario devengado:'),1,0,'L',1);
            $pdf->SetFont('Arial','',7);
            $pdf->Cell(45,5,utf8_decode($candidato['data'][0]['salario_dev']),1,2,'L',0);
            $pdf->SetX(10);
            ...
            $pdf->SetFont('Arial','B',8);
            $pdf->Cell(45,5,utf8_decode('Cargo a Desempeñar:'),1,0,'L',1);
            $pdf->SetFont('Arial','',7);
...
*/            
            ...
            $pdf->Ln(8);
            $pdf->SetFont('Arial','B',9);
            $pdf->Cell(190,5,utf8_decode('1. FAMILIA DE ORIGEN Y PERSONAS CON QUIENES VIVE ACTUALMENTE EL CANDIDATO'),0,0,'L',0);
            $pdf->Ln(8);
            ...
            foreach ($forAcedemica['data'] as $row) {
                $pdf->SetFillColor(207,207,207);
                $pdf->SetFont('Arial','B',8);
                $pdf->Cell(25,5,utf8_decode("Nivel académico"),1,0,'L',1);
                $pdf->SetFont('Arial','',8);
                $pdf->Cell(30,5,utf8_decode($row['descripcion_niv_escol']),1,0,'L',0);
                $pdf->SetFont('Arial','B',8);
...
*/
         ...
         $pdf->Ln(15); 
          ...
          // $pdf->SetAligns(array('L','C','C','C'));
           ...
           // $pdf->Cell(30,5,utf8_decode("Aclaraciones del estado de la vivienda :"),1,0,'L',1);
            ...
            $varXA = $pdf->GetX();
            $varYA = $pdf->GetY();
            $pdf->Row(array(utf8_decode(' '),(utf8_decode($sector['data'][0]['tmp_en_vivienda']))));
        ...
        /*    $varXD = $pdf->GetX();
            ...
            $pdf->SetXY(10,$varYA);
            $pdf->SetFont('Arial','B',8);
            $pdf->Cell(52,5,utf8_decode("Disponibilidad de zonas verdes:    "),0,0,'L',0);
            //$pdf->SetFont('Arial','',8);
       ...
       /*    $varXD = $pdf->GetX();
           ...
           $pdf->SetAligns(array('L','L','L'));
            ...
            $pdf->Ln(8);
            $pdf->SetFont('Arial','B',8);
            $pdf->Cell(190,5,utf8_decode('A. INGRESOS Y EGRESOS DEL CANDIDATO Y LA FAMILIA CON LA QUE CONVIVE'),0,0,'L',0);
            
            $pdf->Ln(6);
            $pdf->SetFillColor(207, 207, 207);
            $pdf->SetFont('Arial','B',7);
            $pdf->Cell(95,5,utf8_decode("Ingresos Mensuales del candidato y familia con la que convive"),1,0,'C','1');
            $pdf->SetFont('Arial','B',7);
            $pdf->Cell(95,5,utf8_decode("Egresos Mensuales familiares del candidato y familia con la que convive"),1,1,'C',1);
           ...
           // $pdf->Cell(40,5,utf8_decode("PREGUNTAS"),1,0,'C',1);
            ...
            $dimPregSalud= CtrDimRespuestas::findAllVariables($id_solicitud,5);
          ...
          //  $pdf->Ln(8);
            ...
            $pdf->SetFillColor(174, 214, 241);
            $pdf->SetFont('Arial','B',10);
            $pdf->Cell(190,5,utf8_decode('V. DIMENSIÓN ACTITUD Y COMPROMISO DEL CANDIDATO CON EL PROCESO'),0,0,'C',1);
            $pdf->Ln(8);
