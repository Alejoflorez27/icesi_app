<?php
function infPoligrafiaRutina($pdf, $id_solicitud, $id_servicio)
{
    ...
    $pdf->MultiCell(190, 5, utf8_decode("La empresa ". utf8_decode($solicitud['data'][0]['razon_social'])." solicita evaluación poligráfica con el propósito de determinar si ". utf8_decode($candidato['data'][0]['nombre'])." ".utf8_decode($candidato['data'][0]['apellido']).
                                        " cumple con el perfil de confiabilidad que desempeña en el cargo de: ". 
...
/*
    ...
    $pdf->SetX(10);
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(0, 5, utf8_decode("ACTITUD FRENTE AL PROCESO DE EVALUACIÓN POLIGRAFÍA"), 0, 0, 'C', 1);
    $pdf->SetX(10);
    $pdf->Ln(5);
...
/*
    ...
    $dimFamiliar = CtrDimRespuestas::descripcionCompromisodimensionPolPre($id_solicitud, 5, $id_servicio);
...
*/
    ...
    if ($obsFamilia['data'] != NULL) {
        ...
        $pdf->SetFont('Arial', '', 8);
        $pdf->Ln(6);
        $pdf->MultiCell(190, 5, utf8_decode($obsFamilia['data']['observacion']), 0, 'J', 0);
    }
...
/*
    ...
    foreach ($familia['data'] as $row) {
        ...
        $pdf->Row(array(
            ...
            utf8_decode(utf8_decode($row['descripcion_financiero'])),
        ));
    }
*/
    ...
    if ($countAcademico < 1) {
        ...
        $pdf->Cell(190, 5, utf8_decode('EMPLEO ACTUAL'), 0, 0, 'C', 0);
        $pdf->Ln(5);
        ...
        $comentario = CtrObservaciones::observacionById($id_solicitud, $id_servicio, "obs_formacion_pol_rutina");
        //print_r($comentario);
        $pdf->Ln(4);
        $pdf->SetFont('Arial', 'B', 8);
        ...
        $pdf->Ln(5);
        $pdf->SetFont('Arial', '', 8);
        $pdf->multiCell(190, 5, utf8_decode($comentario['data']['observacion']), 0, 'L', 0);
    }
    ...
    if ($countlaboral < 1) {
        ...
        foreach ($laboral['data'] as $row) {
            ...
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(50, 5, utf8_decode($row['nombre_empresa']), 1, 0, 'L', 0);
            $pdf->SetFont('Arial', 'B', 8);
            $pdf->Cell(50, 5, utf8_decode("Cargo inicial desempeñado/Área"), 1, 0, 'L', 1);
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(44, 5, utf8_decode($row['cargo_ingreso']), 1, 0, 'L', 0);
            ...
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(69, 5, utf8_decode($row['fch_ingreso']), 1, 0, 'L', 0);
            $pdf->SetFont('Arial', 'B', 8);
            /*$pdf->Cell(25, 5, utf8_decode("Fecha de Retiro"), 1, 0, 'L', 1);
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(19, 5, utf8_decode($row['fch_retiro']), 1, 0, 'L', 0);*/
            ...
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(50, 5, utf8_decode($row['tmp_total_laborado']), 1, 0, 'L', 0);
            /*$pdf->SetFont('Arial', 'B', 8);
            $pdf->Cell(25, 5, utf8_decode("Motivo de Retiro"), 1, 0, 'L', 1);
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(69, 5, utf8_decode($row['tipo_retiro']), 1, 0, 'L', 0);
            
            $pdf->Ln(5);*/

            $pdf->SetFont('Arial', 'B', 8);
            $pdf->Cell(25, 5, utf8_decode("Ciudad"), 1, 0, 'L', 1);
            $pdf->SetFont('Arial', '', 7);
            $pdf->Cell(69, 5, utf8_decode($row['id_ciudad_act']), 1, 0, 'L', 0);
            $pdf->Ln(5);
...
*/
    ...
    //ilicitos
    $pdf->multiCell(190, 5, utf8_decode("ÁREA 1: PREGUNTAS DE VERIFICACIÓN DE ILÍCITOS EN EL EMPLEO ACTÚAL"), 1, 'L', 1);
/*
    $ilicitos = CtrPreguntaSaludPolPre::findAllByCombo('tipo_pregunta_ilicitos_pol_rutina', $id_solicitud, $id_servicio);
    $descripcionIlicitos= '';
...
*/
    ...
    $ilicitos = CtrPreguntaSaludPolPre::respByCategoria('tipo_pregunta_ilicitos_pol_rutina', $id_solicitud, $id_servicio);
    $pdf->SetFont('Arial', 'B', 8);
    ...
    $comentariosIlicitos = CtrObservaciones::observacionById($id_solicitud, $id_servicio,'obs_ilicito_pol_rutina');
    ...
    $pdf->SetFillColor(174, 214, 241);
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->multiCell(190, 5, utf8_decode("ÁREA 2: SANCIONES LABORALES"), 1, 'L', 1);

    $comentariosSanciones = CtrPreguntaSancionLaboralPolRutina::sancionPolRutinaById($id_solicitud, $id_servicio);

    $pdf->SetWidths(array(140,50));
    $pdf->SetAligns(array('L', 'C'));
    $pdf->SetFont('Arial', '', 9);
    $pdf->Row(array(utf8_decode("¿Ha recibido sanciones escritas memorandos, llamados de atención o".
    " ha sido objeto de investigaciones administrativas o disciplinarias por actos cometidos en contra de la empresa?")
    , utf8_decode($comentariosSanciones['data']['pregunta_uno'])));

    if ($comentariosSanciones['data']['pregunta_uno'] == 'Si' && $comentariosSanciones['data']['pregunta_dos'] != null) {
        $pdf->multiCell(190, 5, utf8_decode("Explicación: ". $comentariosSanciones['data']['pregunta_dos']), 1, 'L', 0);
    }
    ...
    $pdf->Row(array(utf8_decode("¿Ha recibido sanciones escritas memorandos, llamados de atención o".
    " ha sido objeto de investigaciones administrativas o disciplinarias por actos cometidos en contra de la empresa?")
    , utf8_decode($comentariosSanciones['data']['pregunta_tres'])));

    if ($comentariosSanciones['data']['pregunta_tres'] == 'Si' && $comentariosSanciones['data']['pregunta_cuatro'] != null) {
        $pdf->multiCell(190, 5, utf8_decode("Explicación: ". $comentariosSanciones['data']['pregunta_cuatro']), 1, 'L', 0);
    }
    
/*
    $sanciones = CtrPreguntaSaludPolPre::findAllByCombo('tipo_pregunta_sanciones_pol_rutina', $id_solicitud, $id_servicio);
    $descripcionSanciones= '';

    foreach ($sanciones['data'] as $row) {
        $descripcionSanciones .= $row['descripcion']. ', ' ;
    }
    $pdf->SetWidths(array(50,140));
    $pdf->SetAligns(array('L', 'L'));
    $pdf->SetFont('Arial', '', 9);
    $pdf->Row(array(utf8_decode("Redacción complementaria"), utf8_decode($descripcionSanciones)));
*/
    //ESTO ES CAMBIAR SEPARADO DE COMAS POR SI Y NO

    if ($comentariosSanciones['data']['pregunta_uno'] == 'Si') {
        $sanciones = CtrPreguntaSaludPolPre::respByCategoria('tipo_pregunta_sanciones_pol_rutina', $id_solicitud, $id_servicio);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(140, 5, utf8_decode("Preguntas"), 1, 0, 'C', 1);
    ...
    //Admiciones por cada Area Academico
    $admisiones = CtrAdmisionesPolPre::admisionesPolPreByIdItem($id_solicitud, $id_servicio,'Sanciones Laborales');
    $admitio = '';
    if (($admisiones['data']['resumen']!= '') && ($admisiones['data']['admitio'] == 1)) {
        $admitio = "Admitio: ".$admisiones['data']['resumen'];
    }else{
    ...
    $pdf->multiCell(190, 4, utf8_decode("Comentarios del examinador: ".($comentariosSanciones['data']['pregunta_cinco'])."\n".$admitio), 1, 'L', 0);

    //$pdf->multiCell(190, 5, utf8_decode("Comentarios del examinador: ".($comentariosSanciones['data']['pregunta_cinco'])), 1, 'L', 0);

    //alcohol
    $pdf->SetFillColor(174, 214, 241);
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->multiCell(190, 5, utf8_decode("ÁREA 3: CONSUMO DE ALCOHOL"), 1, 'L', 1);
/*
    $alcohol = CtrPreguntaSaludPolPre::findAllByCombo('tipo_alcohol_pol_rutina', $id_solicitud, $id_servicio);
    $descripcionAlcohol= '';
...
*/
    //ESTO ES CAMBIAR SEPARADO DE COMAS POR SI Y NO
    $alcohol = CtrPreguntaSaludPolPre::respByCategoria('tipo_alcohol_pol_rutina', $id_solicitud, $id_servicio);
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(140, 5, utf8_decode("Preguntas"), 1, 0, 'C', 1);
    ...
    //FIN DE TABLA DE AREA DE SI Y NO
    $comentariosAlcohol = CtrObservaciones::observacionById($id_solicitud, $id_servicio,'obs_consumo_alcohol_pol_rutina');

    //Admiciones por cada Area Academico
    $admisiones = CtrAdmisionesPolPre::admisionesPolPreByIdItem($id_solicitud, $id_servicio,'consumo de Alchol');
    $admitio = '';
    if (($admisiones['data']['resumen']!= '') && ($admisiones['data']['admitio'] == 1)) {
        $admitio = "Admitio: ".$admisiones['data']['resumen'];
    }else{
    ...
    $pdf->SetFillColor(174, 214, 241);
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->multiCell(190, 5, utf8_decode("ÁREA 4: CONSUMO SUSTANCIAS ALUCINÓGENAS"), 1, 'L', 1);
/*
    $marihuana = CtrPreguntaSaludPolPre::findAllByCombo('tipo_sustancias_pol_rutina', $id_solicitud, $id_servicio);
...
*/
    //ESTO ES CAMBIAR SEPARADO DE COMAS POR SI Y NO
    $marihuana = CtrPreguntaSaludPolPre::respByCategoria('tipo_sustancias_pol_rutina', $id_solicitud, $id_servicio);
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(140, 5, utf8_decode("Preguntas"), 1, 0, 'C', 1);
    ...
    //FIN DE TABLA DE AREA DE SI Y NO
    $comentariosMarihuana = CtrObservaciones::observacionById($id_solicitud, $id_servicio,'obs_sustancias_alcohol_pol_rutina');

    //Admiciones por cada Area Academico
    $admisiones = CtrAdmisionesPolPre::admisionesPolPreByIdItem($id_solicitud, $id_servicio,'consumo de Sustancias');
    $admitio = '';
    if (($admisiones['data']['resumen']!= '') && ($admisiones['data']['admitio'] == 1)) {
        $admitio = "Admitio: ".$admisiones['data']['resumen'];
    }else{
    ...
    $pdf->multiCell(190, 4, utf8_decode("Comentarios del examinador: ".($comentariosMarihuana['data']['observacion'])."\n".$admitio), 1, 'L', 0);
...
/*
    ...
    $pdf->multiCell(190, 5, utf8_decode("Comentarios del examinador: ".($comentariosProduccion['data']['observacion'])), 1, 'L', 0);
    ...
    $pdf->multiCell(190, 5, utf8_decode("Comentarios del examinador: ".($comentariosLudopatia['data']['observacion'])), 1, 'L', 0);
*/

    //Vínculo con personas al margen de la ley
    $pdf->SetFillColor(174, 214, 241);
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->multiCell(190, 5, utf8_decode("ÁREA 5: VÍNCULOS CON PERSONAS AL MARGEN DE LA LEY"), 1, 'L', 1);
    //new
    $pdf->SetFont('Arial', 'B', 8);
    ...
    $pdf->SetFont('Arial', '', 9);
    $vinculoResp = '';
    if ($vinculoPersonas['data']['pregunta_uno'] == 'A') {
        $vinculoResp = 'Aceptó';
    } else {
        $vinculoResp = 'Negó';
    }
...
/*
    $descripcionTipoVinculo= '';

    foreach ($tipoVinculos['data'] as $row) {
        $descripcionTipoVinculo .= $row['descripcion']. ', ' ;
    }
    $pdf->SetWidths(array(50,140));
    $pdf->SetAligns(array('L', 'L'));
    $pdf->SetFont('Arial', '', 9);
    $pdf->Row(array(utf8_decode("Quien tiene vinculos"), utf8_decode($descripcionTipoVinculo)));
*/
    ...
    if ($vinculoPersonas['data']['pregunta_tres'] == 'A') {
        ...
        if ($getYa > $getYb) {
            ...
            if ($vinculoPersonas['data']['pregunta_cuatro'] != null || $vinculoPersonas['data']['pregunta_cuatro'] != '') {
                $pdf->SetWidths(array(102,88));
                $pdf->SetAligns(array('L', 'L'));
                $pdf->Row(array(utf8_decode("¿Ha recibido usted algún beneficio, producto de actividades ilícitas?"), utf8_decode($vinculoPersonas['data']['pregunta_cuatro'])));
            }
        } else {
        $vinculoResp = 'Negó';
    }
    ...
    $pdf->multiCell(190, 4, utf8_decode("Comentarios del examinador: ".($vinculoPersonas['data']['pregunta_cinco'])."\n".$admitio), 1, 'L', 0);

    //$pdf->multiCell(190, 5, utf8_decode("Comentarios del examinador: ".($vinculoPersonas['data']['pregunta_cinco'])), 1, 'L', 0);
...
/*
    ...
    $pdf->multiCell(190, 5, utf8_decode("Comentarios del examinador: ".($comentariosComision['data']['observacion'])), 1, 'L', 0);
    ...
    $pdf->multiCell(190, 5, utf8_decode("Comentarios del examinador: ".($comentariosAntecedentes['data']['observacion'])), 1, 'L', 0);
    ...
    //comentarios del examinado
    $pdf->multiCell(190, 5, utf8_decode("Comentarios del examinador: ".($infoFalsa['data']['pregunta_dos'])), 1, 'L', 0);
    ...
    $pdf->multiCell(190, 5, utf8_decode("Comentarios del examinador: ".($comentariosInfiltracion['data']['observacion'])), 1, 'L', 0);
    ...
    $pdf->Ln(8);
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Cell(190, 5, utf8_decode('¿COMO CONOCIO EL PROCESO?'), 0, 0, 'C', 0);
    $pdf->Ln(8);
    ...
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(120, 5, utf8_decode("¿Tiene algun vinculo con alguien que trabaje en una empresa del mismo sector?"), 1, 0, 'L', 1);
    $pdf->SetFont('Arial', '', 7);
    $pdf->Cell(70, 5, utf8_decode($trabajSector), 1, 0, 'L', 0);
...
*/
    ...
    $pdf->Ln(3);
    $resultado_pol_pre = CtrObservaciones::observacionById($id_solicitud, $id_servicio, 'resultado_pol_rutina');
